pipeline {
    ...
    environment {
        ...
        ECR_REPOSITORY = "smooth/driving-analysis-service"
        IMAGE_TAG = "${new Date().format('yyyyMMdd')}-${sh(script: 'git rev-parse --short HEAD', returnStdout: true).trim()}"
        AWS_DEFAULT_REGION = "ap-northeast-2"
        K8S_NAMESPACE = "driving"
        SECRET_NAME = "driving-analysis-service-secret"
    }
        ...
        stage('Build with application.yaml') {
            steps {
                script {
                    ...
                    withCredentials([file(credentialsId: 'driving-analysis-application-yml-file', variable: 'APPLICATION_YAML_FILE')]) {
        ...
        stage('Create Kubernetes Secret') {
            steps {
                script {
                    ...
                    withCredentials([
                        file(credentialsId: 'driving-analysis-application-yml-file', variable: 'SECRET_FILE_PATH'),
            ...
            script {
                ...
                withCredentials([string(credentialsId: 'discord-webhook-url', variable: 'DISCORD_WEBHOOK_URL')]) {
                    discordSend description: "Jenkins Job: ${env.JOB_NAME}",
                        ...
                        link: env.BUILD_URL,
                        title: "Driving Analysis Service Build Result",
