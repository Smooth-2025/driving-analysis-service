...
public class MilestoneController {
    ...
    public ResponseEntity<ApiResponse<MilestoneReportResponseDto>> getStamp() {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("스탬프 정보를 조회했습니다.", response));
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<MilestoneReportResponseDto>> setRead(@PathVariable String reportId) {
        // 인증 확인 (사용자 권한 체크용)
        ...
        return ResponseEntity.ok(ApiResponse.success("읽음 상태가 업데이트되었습니다.", response));
    }
}
